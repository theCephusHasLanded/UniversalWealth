...
var r = function(t2) {
  ...
  var e2 = n();
  if (e2) {
    if (t2 < e2.domInteractive)
      return "loading";
    if (0 === e2.domContentLoadedEventStart || t2 < e2.domContentLoadedEventStart)
      return "dom-interactive";
    if (0 === e2.domComplete || t2 < e2.domComplete)
...
var H = function(t2) {
  if (q.forEach(function(e3) {
    ...
    if (n2 || B.length < 10 || t2.duration > e2.latency) {
      if (n2)
        t2.duration > n2.latency ? (n2.entries = [t2], n2.latency = t2.duration) : t2.duration === n2.latency && t2.startTime === n2.entries[0].startTime && n2.entries.push(t2);
      else {
...
var nt = function(t2, e2) {
  for (var n2, r2 = [], i2 = 0; n2 = U[i2]; i2++)
    if (!(n2.startTime + n2.duration < t2)) {
      if (n2.startTime > e2)
        break;
      r2.push(n2);
    }
